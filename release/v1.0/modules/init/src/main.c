 ...
 *
 * Prints welcome banner, spawns the shell module, and exits.
 * The kernel runs spawned modules after init completes.
 */
...
void _start(void)
{
    ...
    print_str("Spawning shell... ");
    int shell_id = spawn_module("shell");
    if (shell_id >= 0) {
        print_str("ok (id=");
        /* Print shell_id as decimal */
        if (shell_id == 0) {
            print_str("0");
        } else {
            char buf[12];
