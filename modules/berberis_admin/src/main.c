...
int ber_init(void){if(init)return -1;st.n_berp=0;st.n_bere=0;st.n_ber2=0;st.n_ac=0;st.n_mk=0;st.t_f1=0;st.t_f2=0;st.t_f3=0;st.t_f4=0;st.t_f5=0;for(int i=0;i<N;i++)berps[i].active=0;for(int i=0;i<N-2;i++)berss[i].active=0;for(int i=0;i<N-4;i++)bervss[i].active=0;for(int i=0;i<N-6;i++)beras[i].active=0;for(int i=0;i<N-6;i++)bermks[i].active=0;init=1;ps("[BER] Berberis initialized\n");return 0;}
...
ps("Berberis planning...\n");for(int i=0;i<N;i++){int t=(i%5)+1,c=(i%4)+1;ber_planning(t,c,979+(i*17),968+(i*14),948+(i*10),930+(i*6),2020+(i%5));}
ps("\nBerberis execution...\n");for(int i=0;i<N-2;i++){int t=(i%4)+1,c=(i%5)+1;ber_execution(t,c,968+(i*15),957+(i*12),939+(i*8),926+(i*5),2021+(i%4));}
ps("\nBerberis evaluation...\n");for(int i=0;i<N-4;i++){int t=(i%4)+1,c=(i%5)+1;ber_evaluation(t,c,960+(i*13),949+(i*10),933+(i*7),922+(i*4),2022+(i%3));}
ps("\nBerberis accessories...\n");for(int i=0;i<N-6;i++){int t=(i%4)+1,c=(i%5)+1;ber_accessory(t,c,952+(i*11),943+(i*9),929+(i*6),919+(i*3),2023+(i%2));}
ps("\nBerberis marketing...\n");for(int i=0;i<N-6;i++){int t=(i%4)+1,c=(i%5)+1;ber_market(t,c,946+(i*9),937+(i*7),924+(i*5),916+(i*3),2024);}
