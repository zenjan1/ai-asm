 ...
 * Subsystems: Content Addressing, Sharded Storage (XOR parity),
 *             DHT (Kademlia), Content Routing, Data Persistence,
 *             Bandwidth Management. Pure C, no stdlib.
 */
#include <stddef.h>
...
static void my_strncpy(char *d, const char *s, unsigned int n) {
    unsigned int i; for (i = 0; i < n && s[i]; i++) d[i] = s[i];
    for (; i < n; i++) d[i] = '\0';
}
...
static void print_hex(const unsigned char *d, unsigned int n) {
    ...
    for (unsigned int i = 0; i < n; i++) {
        char c[3] = { h[d[i] >> 4], h[d[i] & 0xf], '\0' }; host_print(c);
    }
}
...
static unsigned int shard_split(const unsigned char *data, unsigned int len,
    ...
    for (unsigned int i = 0; i < cnt; i++) {
        ...
        out[i].size = rem < csz ? rem : csz; out[i].parity_shard = 0;
...
static void shard_compute_parity(shard_t *shards, unsigned int dc, unsigned int pc) {
    for (unsigned int p = 0; p < pc; p++) {
        ...
        shards[pi].shard_id = pi; shards[pi].parity_shard = 1; shards[pi].size = shards[0].size;
        for (unsigned int j = 0; j < shards[0].size; j++) {
            unsigned char v = 0;
            for (unsigned int d = 0; d < dc; d++) v ^= shards[d].data[j];
            v ^= (unsigned char)((p + 1) * 0x37); shards[pi].data[j] = v;
        }
...
static int shard_reconstruct(shard_t *shards, unsigned int dc, unsigned int pc) {
    ...
    for (unsigned int i = 0; i < dc; i++) {
        ...
        unsigned int pi = dc;
        for (unsigned int p = 0; p < pc; p++) if (shards[dc+p].size > 0) { pi = dc+p; break; }
        if (pi >= dc + pc) return -2;
        shards[i].size = shards[pi].size; shards[i].shard_id = i; shards[i].parity_shard = 0;
        for (unsigned int j = 0; j < shards[i].size; j++) {
            unsigned char v = shards[pi].data[j] ^ (unsigned char)(((pi-dc)+1)*0x37);
...
static void dht_init(const node_id_t *local_id) {
    ...
    for (int i = 0; i < MAX_DHT_BUCKETS; i++) {
        g_dht.buckets[i].count = 0; g_dht.buckets[i].max_entries = K_BUCKET_SIZE;
    }
}
static unsigned int dht_xor_distance(const node_id_t *a, const node_id_t *b) {
    unsigned int d = 0;
    for (unsigned int i = 0; i < NODE_ID_LEN; i++) { d = d * 31u + (a->id[i] ^ b->id[i]); }
    return d;
}
...
static int dht_add_node(dht_table_t *dht, const dht_entry_t *node) {
    ...
    b->entries[b->count] = *node; b->entries[b->count].k_bucket_index = bi;
    b->entries[b->count].last_seen = now_ts; b->count++; return (int)bi;
}
...
static int routing_announce(provider_index_t *idx, const cid_t *cid,
    ...
    idx->records[i].timestamp = now_ts; idx->records[i].load = 0; return (int)i;
}
...
static int store_put(block_store_t *s, const cid_t *cid, const unsigned char *data, unsigned int sz) {
    ...
    i = (int)s->count++; s->blocks[i].cid = *cid;
    for (unsigned int j = 0; j < sz && j < BLOCK_DATA_SZ; j++) s->blocks[i].data[j] = data[j];
    s->blocks[i].size = sz; s->blocks[i].pinned = 0; s->blocks[i].replication_count = 0;
    s->blocks[i].created_at = now_ts; s->blocks[i].last_access = now_ts; return i;
}
static int store_get(block_store_t *s, const cid_t *cid) {
    int i = store_find(s, cid); if (i >= 0) s->blocks[i].last_access = now_ts; return i;
}
static int store_pin(block_store_t *s, const cid_t *cid) {
    int i = store_get(s, cid); if (i < 0) return -1; s->blocks[i].pinned = 1; return i;
}
static int store_unpin(block_store_t *s, const cid_t *cid) {
    int i = store_get(s, cid); if (i < 0) return -1; s->blocks[i].pinned = 0; return i;
}
...
static int bw_schedule_transfer(bw_manager_t *bw, const transfer_t *t) {
    ...
    if (bw->active < bw->cfg.max_concurrent_transfers) {
        bw->transfers[i].state = TS_RUNNING; bw->transfers[i].started_at = now_ts; bw->active++;
    }
    return (int)i;
}
...
static int bw_resume_interrupted(transfer_t *t) {
    ...
    t->state = TS_RUNNING; t->started_at = now_ts; return (int)t->bytes_transferred;
}
...
static void run_test(void) {
    ...
    const unsigned char d1[] = "Hello decentralized world!";
    cid_t c1; content_address(d1, sizeof(d1)-1, &c1);
    print_str("  CID: "); print_hex(c1.mh.digest, 16); print_str("...\n");
    print_str("  verify ok: "); print_str(content_verify(d1, sizeof(d1)-1, &c1) ? "PASS" : "FAIL"); print_str("\n");
    const unsigned char bad[] = "tampered";
    print_str("  verify bad: "); print_str(content_verify(bad, sizeof(bad)-1, &c1) ? "PASS" : "FAIL");
    print_str(" (expected)\n");
    cid_t c2; content_address(d1, sizeof(d1)-1, &c2);
    ...
    unsigned char bd[64]; for (int i = 0; i < 64; i++) bd[i] = (unsigned char)(i*7+3);
    shard_t shards[MAX_SHARDS];
    unsigned int ns = shard_split(bd, 64, sc, shards);
    print_str("  shards: "); print_int(ns); print_str("\n");
    shard_compute_parity(shards, ns, sc.parity_count);
    print_str("  parity: "); print_int(sc.parity_count); print_str("\n");
    print_str("  shard[0] verify: "); print_str(shard_verify(&shards[0]) ? "PASS" : "FAIL"); print_str("\n");
    shards[1].size = 0; shards[2].size = 0;
    print_str("  reconstructed: "); print_int(shard_reconstruct(shards, ns, sc.parity_count)); print_str("\n");
    print_str("  shard[1] verify: "); print_str(shard_verify(&shards[1]) ? "PASS" : "FAIL"); print_str("\n\n");
    ...
    for (int p = 0; p < 6; p++) {
        ...
        e.port = 4000 + p;
        print_str("  add p="); print_int(p); print_str(" bkt="); print_int(dht_add_node(&g_dht, &e)); print_str("\n");
    }
    node_id_t tgt; for (int i = 0; i < NODE_ID_LEN; i++) tgt.id[i] = (unsigned char)(i*3+5);
    dht_entry_t cl[K_BUCKET_SIZE];
    print_str("  closest: "); print_int(dht_find_closest(&g_dht, &tgt, 3, cl)); print_str("\n");
    tick(100); dht_expire_stale(&g_dht, EXPIRE_TIMEOUT); print_str("\n");

    print_str("[4] Content Routing\n");
    routing_init();
    unsigned char pv1[NODE_ID_LEN], pv2[NODE_ID_LEN], a1[ADDR_LEN];
    for (int i = 0; i < NODE_ID_LEN; i++) { pv1[i] = 0xA1; pv2[i] = 0xB2; }
    for (int i = 0; i < ADDR_LEN; i++) a1[i] = (unsigned char)(10+i);
    print_str("  announce: "); print_int(routing_announce(&g_pidx, &c1, pv1, a1));
    print_str(", "); print_int(routing_announce(&g_pidx, &c1, pv2, a1)); print_str("\n");
    provider_record_t fpv[MAX_PROVIDERS];
    unsigned int nf = routing_find_providers(&g_pidx, &c1, 10, fpv);
    print_str("  providers: "); print_int(nf); print_str(" lb="); print_int(routing_load_balance(fpv, nf)); print_str("\n");
    routing_remove_provider(&g_pidx, &c1, pv1);
    print_str("  after rm: "); print_int(routing_find_providers(&g_pidx, &c1, 10, fpv)); print_str("\n\n");

    print_str("[5] Data Persistence\n");
    store_init();
    int bi = store_put(&g_store, &c1, d1, sizeof(d1)-1);
    print_str("  put="); print_int(bi); print_str(" get="); print_int(store_get(&g_store, &c1)); print_str("\n");
    store_pin(&g_store, &c1); print_str("  pinned="); print_int(g_store.blocks[bi].pinned); print_str("\n");
    store_replicate(&g_store, &c1, REPL_FACTOR);
    print_str("  rep="); print_int(g_store.blocks[bi].replication_count); print_str("\n");
    store_unpin(&g_store, &c1); print_str("  unpinned="); print_int(g_store.blocks[bi].pinned); print_str("\n\n");
    ...
    transfer_t tx; tx.cid = c1; tx.direction = DIR_UPLOAD; tx.total_bytes = 8192; tx.priority = 5;
    int ti = bw_schedule_transfer(&g_bw, &tx);
    print_str("  sched="); print_int(ti); print_str("\n");
    g_bw.transfers[ti].bytes_transferred = 2048; g_bw.transfers[ti].elapsed_ms = 500;
    print_str("  rate="); print_int(bw_compute_rate(&g_bw.transfers[ti], 500)); print_str(" B/s\n");
    print_str("  throttle="); print_int(bw_throttle(&g_bw, &g_bw.transfers[ti])); print_str("\n");
    g_bw.transfers[ti].state = TS_INTERRUPTED;
    print_str("  resume_off="); print_int(bw_resume_interrupted(&g_bw.transfers[ti])); print_str("\n\n");
