/* setfont_v2: console font loading simulation (enhanced) */
...
void _start(void) {
    ...
    if (argv_len > 0) {
        ...
        if (i < argv_len && argv[i] == '-' && i + 1 < argv_len && argv[i + 1] == 'h') {
            print_str("Usage: setfont_v2 [ font_file ] [ -h height ]\n");
            print_str("Load console font with enhanced options.\n");
            print_str("\nOptions:\n");
            print_str("  -h  show this help\n");
            print_str("  font_file  PSF font file to load\n");
            print_str("  -h height  set font height\n");
            host_exit(1);
        }
    }

    print_str("setfont_v2: console font loading simulation (enhanced)\n\n");
