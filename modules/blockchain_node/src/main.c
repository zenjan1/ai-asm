...
static void ecdsa_sign(const unsigned char *pk, const unsigned char *msg, unsigned char *sig) {
    unsigned int k = 0;
    for (unsigned int i = 0; i < 32; i++) k = k*31+(unsigned int)pk[i];
    for (unsigned int i = 0; i < 32; i++) k ^= ((unsigned int)msg[i])<<(i&3);
    for (unsigned int i = 0; i < 32; i++) {
        sig[i]=(unsigned char)((k^(unsigned int)pk[i])&0xFF); k=(k<<3)|(k>>29); k^=0x9E3779B9; }
    for (unsigned int i = 0; i < 32; i++) {
        sig[32+i]=(unsigned char)((k^(unsigned int)msg[i])&0xFF); k=(k<<5)|(k>>27); k^=0x5A827999; } }
...
typedef struct { unsigned char tx_id[32]; unsigned char from[20]; unsigned char to[20];
    ...
    unsigned int nonce; unsigned char signature[64]; int state; } transaction_t;
...
static void tx_sign(transaction_t *tx, const unsigned char *pk) {
    unsigned char msg[32]; sha256((const unsigned char *)tx, 64, msg);
    ecdsa_sign(pk, msg, tx->signature); }
static int tx_verify(const transaction_t *tx) {
    unsigned char msg[32], pub[33]; mcpy(pub, tx->from, 20);
    for (int i = 20; i < 33; i++) pub[i] = 0;
    sha256((const unsigned char *)tx, 64, msg);
    return ecdsa_verify(pub, msg, tx->signature); }
static int tx_pool_add(tx_pool_t *p, const transaction_t *tx) {
    if (p->count >= MAX_TX_POOL) return -1;
    for (unsigned int i = 0; i < p->count; i++)
        if (mcmp(p->txs[i].tx_id, tx->tx_id, 32) == 0) return -2;
    p->txs[p->count++] = *tx; return (int)p->count; }
static int tx_check_double_spend(const tx_pool_t *p, const transaction_t *tx) {
    for (unsigned int i = 0; i < p->count; i++)
        if (mcmp(p->txs[i].from, tx->from, 20) == 0 && p->txs[i].nonce == tx->nonce) return 1;
    return 0; }
static void tx_pool_sort_by_fee(tx_pool_t *p) {
    for (unsigned int i = 0; i+1 < p->count; i++)
        for (unsigned int j = i+1; j < p->count; j++)
            if (p->txs[j].fee > p->txs[i].fee) {
                transaction_t t = p->txs[i]; p->txs[i] = p->txs[j]; p->txs[j] = t; } }
static int tx_pool_remove(tx_pool_t *p, const unsigned char *txid) {
    for (unsigned int i = 0; i < p->count; i++)
        if (mcmp(p->txs[i].tx_id, txid, 32) == 0) {
            for (unsigned int j = i; j+1 < p->count; j++) p->txs[j] = p->txs[j+1];
            p->count--; return 1; } return 0; }
...
typedef struct { block_header_t header; transaction_t txs[8]; unsigned int tx_count;
    unsigned char block_hash[32]; } block_t;
typedef struct { block_t blocks[MAX_BLOCKS]; unsigned int chain_length;
    unsigned int difficulty; } blockchain_t;
...
static void blockchain_init(blockchain_t *ch) { memzero(ch, sizeof(blockchain_t)); ch->difficulty = 4;
    ...
    sha256((const unsigned char *)&ch->blocks[0].header, sizeof(block_header_t),
           ch->blocks[0].block_hash); ch->chain_length = 1; }
static void merkle_root(const unsigned char tx_h[][32], unsigned int cnt, unsigned char *out) {
    ...
    while (lc > 1) { unsigned int nx = 0;
        for (unsigned int i = 0; i+1 < lc; i += 2) {
            mcpy(buf, level[i], 32); mcpy(buf+32, level[i+1], 32);
            sha256(buf, 64, level[nx++]); }
        if (lc%2 == 1) { mcpy(level[nx], level[lc-1], 32); nx++; } lc = nx; }
    mcpy(out, level[0], 32); }
static block_t block_create(blockchain_t *ch, transaction_t *txs, unsigned int cnt) {
    ...
    if (tc > 0) { unsigned char th[8][32];
        for (unsigned int i = 0; i < tc; i++)
            sha256((const unsigned char *)&txs[i], 64, th[i]);
...
static int block_validate(const block_t *blk, const blockchain_t *ch) {
    ...
    if (mcmp(blk->header.prev_hash, ch->blocks[ch->chain_length-1].block_hash, 32) != 0) return 0;
    return 1; }
static int blockchain_add_block(blockchain_t *ch, const block_t *blk) {
    if (ch->chain_length >= MAX_BLOCKS) return -1;
    if (!block_validate(blk, ch)) return 0;
...
static int chain_select_best(const blockchain_t *a, const blockchain_t *b) {
    if (a->chain_length != b->chain_length)
        return b->chain_length > a->chain_length ? 1 : 0;
    return mcmp(a->blocks[a->chain_length-1].block_hash,
                b->blocks[b->chain_length-1].block_hash, 32) < 0 ? 0 : 1; }
...
static void block_sync(blockchain_t *local, const blockchain_t *remote) {
    ...
    for (unsigned int i = local->chain_length; i < end; i++)
        local->blocks[i] = remote->blocks[i]; local->chain_length = end; }

/* ====================== Consensus: PoS / DPoS / PBFT / Raft ====================== */
...
static int consensus_pos_select(const validator_t *vals, unsigned int cnt, unsigned char *sel) {
    ...
    for (unsigned int i = 0; i < cnt; i++) if (vals[i].active) total += vals[i].stake;
    if (total == 0) return -1;
...
static unsigned int consensus_dpos_elect(const delegate_t *dels, unsigned int cnt,
                                         unsigned char *win, unsigned int top) {
    unsigned long long sc[8]; unsigned int ord[8], lim = cnt > 8 ? 8 : cnt;
    for (unsigned int i = 0; i < lim; i++) { sc[i] = dels[i].votes; ord[i] = i; }
    for (unsigned int i = 0; i+1 < lim; i++)
        for (unsigned int j = i+1; j < lim; j++)
...
static int consensus_raft_elect(const raft_node_t *nodes, unsigned int cnt) {
    unsigned int vf = 0;
    for (unsigned int i = 0; i < cnt; i++) if (nodes[i].vote_granted) vf++;
    return vf > cnt/2 ? 1 : 0; }
static int consensus_raft_append(const raft_node_t *nodes, unsigned int cnt,
                                 const void *entry, unsigned int esz) {
    unsigned char eh[32]; sha256((const unsigned char *)entry, esz, eh);
    unsigned int acks = 0;
    for (unsigned int i = 0; i < cnt; i++) if (nodes[i].vote_granted) acks++;
    return acks > cnt/2 ? 1 : 0; }

/* ====================== P2P Network ====================== */

typedef struct { unsigned char peer_id[20]; char endpoint[32]; int status;
    unsigned int last_seen; unsigned int blocks_height; } peer_t;
typedef struct { peer_t peers[MAX_PEERS]; unsigned int peer_count;
    unsigned char msg_queue[MAX_MSG_QUEUE][64]; unsigned int msg_count;
    unsigned char local_id[20]; } p2p_network_t;

static void p2p_init(p2p_network_t *net, const unsigned char *node_id) {
    memzero(net, sizeof(p2p_network_t)); mcpy(net->local_id, node_id, 20); }
static int p2p_add_peer(p2p_network_t *net, const unsigned char *pid, const char *ep) {
    if (net->peer_count >= MAX_PEERS) return -1;
    peer_t *p = &net->peers[net->peer_count];
    mcpy(p->peer_id, pid, 20); my_strncpy(p->endpoint, ep, 32);
    p->status = PEER_CONNECTED; p->last_seen = net->peer_count * 10;
    net->peer_count++; return (int)net->peer_count; }
static int p2p_remove_peer(p2p_network_t *net, const unsigned char *pid) {
    for (unsigned int i = 0; i < net->peer_count; i++)
        if (mcmp(net->peers[i].peer_id, pid, 20) == 0) {
            for (unsigned int j = i; j+1 < net->peer_count; j++)
                net->peers[j] = net->peers[j+1];
...
static unsigned int p2p_discover(p2p_network_t *net) { unsigned int found = 0;
    for (unsigned int i = 0; i < net->peer_count; i++)
        if (net->peers[i].status == PEER_DISCONNECTED) {
            net->peers[i].status = PEER_CONNECTED; found++; }
    return found; }
static unsigned int p2p_topology(const p2p_network_t *net) {
    unsigned int connected = 0, syncing = 0;
    for (unsigned int i = 0; i < net->peer_count; i++) {
...
typedef struct { unsigned char tx_id[32]; unsigned int output_idx;
    unsigned char owner[20]; unsigned long long value; int spent; } utxo_t;
...
static int utxo_add(utxo_set_t *s, const unsigned char *txid, unsigned int idx,
                    const unsigned char *owner, unsigned long long val) {
...
static unsigned long long utxo_balance(const utxo_set_t *s, const unsigned char *owner) {
    ...
    for (unsigned int i = 0; i < s->count; i++)
        if (!s->utxos[i].spent && mcmp(s->utxos[i].owner, owner, 20) == 0)
            total += s->utxos[i].value; return total; }
...
typedef struct { unsigned char address[20]; unsigned long long balance;
    unsigned int nonce; unsigned char code_hash[32]; } account_t;
...
static int account_create(account_db_t *db, const unsigned char *addr, unsigned long long bal) {
    if (db->count >= MAX_ACCOUNTS) return -1;
    account_t *a = &db->accounts[db->count];
    mcpy(a->address, addr, 20); a->balance = bal; a->nonce = 0;
    memzero(a->code_hash, 32); db->count++; return (int)db->count; }
static account_t *account_get(account_db_t *db, const unsigned char *addr) {
    for (unsigned int i = 0; i < db->count; i++)
        if (mcmp(db->accounts[i].address, addr, 20) == 0) return &db->accounts[i];
    return (account_t *)0; }
static int account_transfer(account_db_t *db, const unsigned char *from,
                            const unsigned char *to, unsigned long long amt) {
...
/* ====================== Ledger: State Trie (Merkle Patricia) ====================== */

typedef struct trie_node_s { unsigned char key[32]; unsigned int key_len;
    unsigned char value[64]; unsigned int value_len;
    struct trie_node_s *children[16]; unsigned char hash[32]; } trie_node_t;
typedef struct { trie_node_t nodes[MAX_TRIE_NODES]; unsigned int node_count;
    trie_node_t *root; } state_trie_t;

static state_trie_t *trie_create(void) {
...
static int trie_insert(state_trie_t *t, const unsigned char *k, unsigned int kl,
    ...
    if (t->node_count >= MAX_TRIE_NODES) return -1;
    trie_node_t *nd = &t->nodes[t->node_count++]; memzero(nd, sizeof(trie_node_t));
...
static void trie_root_hash(const state_trie_t *t, unsigned char *rh) {
...
typedef struct { unsigned char state_hash[32]; unsigned int block_height;
    unsigned int account_count; unsigned int utxo_count; } snapshot_t;
static snapshot_t snapshot_create(const state_trie_t *trie, unsigned int height,
                                  unsigned int acct_cnt, unsigned int utxo_cnt) {
    snapshot_t snap; memzero(&snap, sizeof(snap));
    trie_root_hash(trie, snap.state_hash);
    snap.block_height = height; snap.account_count = acct_cnt; snap.utxo_count = utxo_cnt;
    return snap; }
static int snapshot_verify(const snapshot_t *snap, const state_trie_t *trie) {
    unsigned char rh[32]; trie_root_hash(trie, rh);
    return mcmp(rh, snap->state_hash, 32) == 0 ? 1 : 0; }

/* ====================== Wallet Management ====================== */

typedef struct { unsigned char address[20]; unsigned char public_key[33];
    unsigned char private_key[32]; unsigned long long balance; unsigned int nonce; } wallet_t;
typedef struct { unsigned char keys[MAX_SIGNATURES][32]; unsigned int threshold;
    unsigned int key_count; unsigned char address[20]; } multisig_wallet_t;
typedef struct { unsigned char master_key[32]; unsigned char chain_code[32];
    char mnemonic[MAX_MNEMONIC_W][12]; unsigned int word_count; unsigned int deriv_index; } hd_wallet_t;
...
static int multisig_create(multisig_wallet_t *mw, const unsigned char keys[][32],
                           unsigned int nkeys, unsigned int threshold) {
...
static int multisig_verify(const multisig_wallet_t *mw, const unsigned char *msg,
                           const unsigned char sigs[][64], unsigned int sig_count) {
    if (sig_count < mw->threshold) return 0;
    unsigned int valid = 0; unsigned char mh[32]; sha256(msg, 32, mh);
    for (unsigned int s = 0; s < sig_count && s < mw->threshold; s++)
        for (unsigned int k = 0; k < mw->key_count; k++) {
            unsigned char pub[33]; mcpy(pub+1, mw->keys[k], 32); pub[0] = 0x02;
...
static hd_wallet_t hd_wallet_create(const char *passphrase) { hd_wallet_t hw; memzero(&hw, sizeof(hw));
    ...
    sha256((const unsigned char *)passphrase, my_strlen(passphrase), seed);
    mcpy(hw.master_key, seed, 32);
    unsigned char cc[32]; sha256(seed, 32, cc); mcpy(hw.chain_code, cc, 32);
    hw.word_count = MAX_MNEMONIC_W;
    for (unsigned int i = 0; i < MAX_MNEMONIC_W; i++) {
        unsigned int idx = rng_next()%12; my_strncpy(hw.mnemonic[i], BIP39_WORDS[idx], 12); }
    hw.deriv_index = 0; return hw; }
static void hd_wallet_derive_child(const hd_wallet_t *hw, unsigned int index,
...
static void run_tests(void) { unsigned int i;
    ...
    print_str("  mined="); print_int(consensus_pow(&blk, 2));
    print_str(", nonce="); print_int((int)blk.header.nonce); print_str("\n\n");
    ...
    print_str("  selected idx="); print_int(consensus_pos_select(vals, 3, sel)); print_str("\n\n");
    ...
    unsigned char elected[40];
    print_str("  elected="); print_int((int)consensus_dpos_elect(dels, 4, elected, 2)); print_str("\n\n");
    ...
    char proposal[] = "block proposal data";
    print_str("  consensus="); print_int(consensus_pbft_run(proposal, 19, pvals, 4)); print_str("\n\n");
    ...
    char entry[] = "log entry";
    print_str("  elected="); print_int(consensus_raft_elect(rnodes, 3));
    ...
    print_str("  pool="); print_int((int)pool.count);
    print_str(", dup="); print_int(tx_pool_add(&pool, &tx1));
    print_str(", dbl_spend="); print_int(tx_check_double_spend(&pool, &tx1));
    tx_pool_sort_by_fee(&pool); print_str("\n\n");
    ...
    block_t fb = block_create(&fork_ch, pool.txs, pool.count);
    blockchain_add_block(&fork_ch, &fb); fork_handle(&chain, &fork_ch); print_str("\n");
    print_str("[8] P2P Network:\n"); unsigned char nid[20];
    for (i = 0; i < 20; i++) nid[i] = (unsigned char)(0x01+i);
    ...
    p2p_add_peer(&net, p1, "192.168.1.10:8333");
    p2p_add_peer(&net, p2, "192.168.1.11:8333");
    char msg[] = "new block announcement";
    print_str("  peers="); print_int((int)net.peer_count);
    ...
    unsigned char txid1[32]; sha256((const unsigned char *)"tx001", 5, txid1);
    utxo_add(&utxo, txid1, 0, fa, 10000);
    unsigned char txid2[32]; sha256((const unsigned char *)"tx002", 5, txid2);
    utxo_add(&utxo, txid2, 0, fa, 5000);
    print_str("  balance="); print_int((int)utxo_balance(&utxo, fa));
    print_str(", spent="); print_int(utxo_spend(&utxo, txid1, 0));
    ...
    account_t *ac = account_get(&adb, fa);
    print_str(", from="); print_int((int)(ac ? ac->balance : 0));
    account_t *bc2 = account_get(&adb, ta);
    print_str(", to="); print_int((int)(bc2 ? bc2->balance : 0)); print_str("\n\n");
    print_str("[11] State Trie:\n"); state_trie_t *trie = trie_create();
    unsigned char k1[] = "alice_balance", v1[] = "50000";
    unsigned char k2[] = "bob_balance", v2[] = "35000";
    ...
    print_str("  height="); print_int((int)snap.block_height);
    print_str(", valid="); print_int(snapshot_verify(&snap, trie)); print_str("\n\n");
    print_str("[13] Wallet:\n"); wallet_t w = wallet_create();
    print_str("  balance="); print_int((int)w.balance);
    unsigned char addr[20]; wallet_derive_address(w.public_key, addr);
    print_str(", addr="); print_hex(addr, 8); print_str("\n\n");
    print_str("[14] Multi-sig Wallet:\n"); unsigned char mkeys[3][32];
    for (unsigned int k = 0; k < 3; k++)
        for (i = 0; i < 32; i++) mkeys[k][i] = (unsigned char)(rng_next() & 0xFF);
    multisig_wallet_t mw; multisig_create(&mw, mkeys, 3, 2);
    unsigned char testmsg[32]; sha256((const unsigned char *)"test", 4, testmsg);
    unsigned char msigs[2][64];
    ecdsa_sign(mkeys[0], testmsg, msigs[0]); ecdsa_sign(mkeys[1], testmsg, msigs[1]);
    ...
    block_t rb = block_create(&remote, pool.txs, 1);
    blockchain_add_block(&remote, &rb); block_sync(&local, &remote);
    print_str("  synced_len="); print_int((int)local.chain_length); print_str("\n\n");
...
void _start(void) {
    char argv_buf[256]; memzero(argv_buf, sizeof(argv_buf));
    host_get_argv(0, 256);
    int ran_test = 0, i = 0;
