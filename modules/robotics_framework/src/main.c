 ...
 * Features: Motion planning (RRT/A*), SLAM, multi-robot coordination,
 *           sensor fusion (IMU/GPS/LiDAR/Cam), control (PID/MPC/RL), simulation
 * Build: clang --target=wasm32-unknown-unknown -nostdlib -Wl,--no-entry \
 *        -Wl,--export-all -o robotics_framework.wasm src/main.c
 */
...
#define MAX_TRAJECTORY 256
/* Robot states */
#define ROBOT_IDLE 0
#define ROBOT_NAVIGATING 1
#define ROBOT_MAPPED 2
#define ROBOT_DOCKED 3
#define ROBOT_ERROR 4
/* Planner types */
#define PLANNER_RRT 0
#define PLANNER_ASTAR 1
#define PLANNER_DSTAR 2
/* Sensor types */
#define SENSOR_IMU 0
#define SENSOR_GPS 1
#define SENSOR_LIDAR 2
#define SENSOR_CAMERA 3
#define SENSOR_ENCODER 4
/* Formation types */
#define FORM_LINE 0
#define FORM_COLUMN 1
#define FORM_CIRCLE 2
#define FORM_VEE 3
...
typedef struct { int len; node_t points[MAX_TRAJECTORY]; double smoothness; } trajectory_t;
...
static trajectory_t current_traj;
...
/* ── Motion Planning: Obstacle checking ── */

static int in_obs(double x, double y) {
    for (int i = 0; i < obs_count; i++)
        if (dist2d(x, y, obstacles[i].x, obstacles[i].y) < obstacles[i].radius) return 1;
    return 0; }
...
static int plan_rrt(double sx, double sy, double gx, double gy, double step, int max_iter) {
    ...
    for (int it = 0; it < max_iter && wp_count < MAX_WAYPOINTS - 1; it++) {
        ...
        for (int j = 1; j < wp_count; j++) {
            double d = dist2d(waypoints[j].x, waypoints[j].y, rx, ry);
            if (d < md) { md = d; nr = j; } }
        /* Steer toward sample */
...
/* ── Motion Planning: A* search algorithm ── */
...
static int plan_astar(double sx, double sy, double gx, double gy, int gw, int gh) {
    ...
    for (int s = 0; s < 64; s++) {
        ...
        for (int dy = -1; dy <= 1; dy++) for (int dx = -1; dx <= 1; dx++) {
            ...
            for (int v = 0; v < vc; v++) if (vis[v] == nid) { skip = 1; break; }
            if (skip) continue;
            /* f(n) = g(n) + h(n) */
            double f = waypoints[cur].cost + dist2d(cx, cy, nx, ny) + dist2d(nx, ny, gx, gy);
            if (f < bf) { bf = f; best = wp_count;
                waypoints[wp_count] = (node_t){nx, ny, cur,
                    waypoints[cur].cost + dist2d(cx, cy, nx, ny)}; }
        }
        if (best == -1) return -1;
...
static int optimize_trajectory(void) {
    if (wp_count < 2 || wp_count > MAX_TRAJECTORY) return -1;
    current_traj.len = wp_count; current_traj.smoothness = 0.0;
    for (int i = 0; i < wp_count; i++) current_traj.points[i] = waypoints[i];
    /* Smooth trajectory using moving average */
    for (int iter = 0; iter < 3; iter++) {
        for (int i = 1; i < wp_count - 1; i++) {
            double ax = (current_traj.points[i-1].x + current_traj.points[i].x +
                         current_traj.points[i+1].x) / 3.0;
            double ay = (current_traj.points[i-1].y + current_traj.points[i].y +
                         current_traj.points[i+1].y) / 3.0;
            if (!in_obs(ax, ay)) {
                current_traj.points[i].x = ax; current_traj.points[i].y = ay; }
        }
    }
    /* Calculate smoothness metric */
    for (int i = 1; i < wp_count - 1; i++) {
        double dx1 = current_traj.points[i].x - current_traj.points[i-1].x;
        double dy1 = current_traj.points[i].y - current_traj.points[i-1].y;
        double dx2 = current_traj.points[i+1].x - current_traj.points[i].x;
        double dy2 = current_traj.points[i+1].y - current_traj.points[i].y;
        current_traj.smoothness += abs_d(dx2 - dx1) + abs_d(dy2 - dy1);
    }
    return 0; }

/* ── SLAM: Feature extraction from sensor scans ── */
...
/* ── SLAM: Map merging from multiple robots ── */
    ...
    for (int i = 0; i < o->count && g_map.count < MAX_MAP_CELLS; i++) {
        ...
        for (int j = 0; j < g_map.count; j++)
            if (abs_d(g_map.cells[j].x - o->cells[i].x) < 0.5 &&
                ...
                g_map.cells[j].occ = (g_map.cells[j].occ + o->cells[i].occ) * 0.5;
                found = 1; m++; break; }
        if (!found) g_map.cells[g_map.count++] = o->cells[i];
    } return m; }

/* ── SLAM: Update map with new sensor data ── */
...
static void slam_update(int rid, double px, double py, double *scan, int n) {
    ...
    for (int i = 0; i < n && g_map.count < MAX_MAP_CELLS; i++)
        g_map.cells[g_map.count++] = (cell_t){
            px + scan[i] * 0.1 * (double)(i % 10 - 5),
            py + scan[i] * 0.1 * (double)(i / 10 - 5),
            0.5 + scan[i] * 0.01}; }

/* ── Multi-Robot: Robot creation ── */
...
/* ── Multi-Robot: Task allocation using Hungarian-like approach ── */

static int allocate_tasks(void) {
    int a = 0;
    for (int i = 0; i < task_count; i++) {
        if (tasks[i].status) continue;
        int best = -1; double bd = 1e18;
        for (int j = 0; j < robot_count; j++) {
            if (robots[j].state != ROBOT_IDLE) continue;
            double d = dist2d(robots[j].pos.x, robots[j].pos.y,
                             tasks[i].target.x, tasks[i].target.y);
            if (d < bd) { bd = d; best = j; }
        }
...
static void update_form(int fi) {
    ...
    for (int i = 0; i < robot_count && f->count < MAX_ROBOTS; i++) {
        ...
        if (f->formation == FORM_LINE) {
            ox = f->center.x + f->count * f->spacing; oy = f->center.y;
        } else if (f->formation == FORM_CIRCLE) {
            double a = 6.28318 * f->count / (robot_count > 0 ? robot_count : 1);
            ox = f->center.x + f->spacing * a; oy = f->center.y + f->spacing * (a - 3.14159);
        } else if (f->formation == FORM_VEE) {
            ox = f->center.x + f->count * f->spacing;
            oy = f->center.y + abs_d((double)f->count - robot_count * 0.5) * f->spacing;
        } else {
            ox = f->center.x; oy = f->center.y + f->count * f->spacing;
        }
        robots[i].pos.x = ox; robots[i].pos.y = oy; f->count++;
    } }

/* ── Sensor Fusion: Registration and weighted fusion ── */
...
static void mpc_opt(double tgt, double cur, double *c) {
    for (int it = 0; it < 10; it++) {
        double e = tgt - mpc_pred(cur, c, mpc_steps);
...
static void physics_step(double dt) {
    for (int i = 0; i < robot_count; i++) {
        robots[i].pos.x += robots[i].vel.x * dt;
        robots[i].pos.y += robots[i].vel.y * dt;
        robots[i].pos.z += robots[i].vel.z * dt;
        robots[i].battery -= 0.01 * dt;
        if (robots[i].battery < 0) robots[i].battery = 0;
    } }

/* ── Simulation: 3D scene visualization ── */

static void render(void) {
    print_str("  Scene: robots="); print_int(robot_count);
    print_str(" obs="); print_int(obs_count);
    print_str(" feats="); print_int(feat_count);
    print_str(" cells="); print_int(g_map.count); print_str("\n");
    for (int i = 0; i < robot_count; i++) {
        print_str("    ["); print_str(robots[i].name);
        print_str("] ("); print_dbl(robots[i].pos.x); print_str(",");
        print_dbl(robots[i].pos.y); print_str(") bat=");
        print_dbl(robots[i].battery); print_str("\n");
    } }

/* ── Help / Info / Status display ── */

static void show_help(void) {
    print_str("robotics_framework v1.0\nUsage: robotics_framework [options]\n");
    print_str("  -h help  -t test  -i info  -s status\n  -m motion  -S slam  -c control  -f formation\n"); }

static void show_info(void) {
    print_str("robotics_framework v1.0\n  Planning: RRT/A*  SLAM: features/merge\n");
    print_str("  Multi-robot: tasks/formations\n  Sensors: IMU/GPS/LiDAR/Cam\n");
    print_str("  Control: PID/MPC/RL  Sim: physics/3D\n"); }

static void show_status(void) {
    print_str("status: robots="); print_int(robot_count); print_str(" obs="); print_int(obs_count);
...
static void test_motion(void) {
    ...
    if (r1 == 0) optimize_trajectory();
    print_str("  trajectory smoothness: "); print_dbl(current_traj.smoothness); print_str("\n");
...
static void test_slam(void) {
    ...
    feat_count = 0;
    print_str("  features: "); print_int(extract_feats(scan, 20, 0.3)); print_str("\n");
...
static void test_multi(void) {
    ...
    int f3 = create_form(FORM_VEE, 15, 15, 1.5); update_form(f3); print_str("  VEE ok\n  OK\n"); }
...
static void test_fusion(void) {
    ...
    print_str("  fused: ("); print_dbl(f.x); print_str(",");
...
static void test_ctrl(void) {
    ...
    mpc_opt(5.0, 0.0, c);
    print_str("  MPC[0]: "); print_dbl(c[0]); print_str("\n");
    rl_episodes = 0;
...
void _start(void) {
    ...
    while (pos < 512 && ap[pos]) { char *a = &ap[pos];
        ...
        else if (!my_strcmp(a, "-i")) i = 1;
        else if (!my_strcmp(a, "-t")) t = 1;
        else if (!my_strcmp(a, "-s")) s = 1;
        else if (!my_strcmp(a, "-m")) m = 1;
        else if (!my_strcmp(a, "-S")) sl = 1;
        else if (!my_strcmp(a, "-c")) c = 1;
        else if (!my_strcmp(a, "-f")) f = 1;
        while (pos < 512 && ap[pos]) pos++; pos++; }
    /* Execute requested action */
    if (h) { show_help(); host_exit(0); }
    if (i) { show_info(); host_exit(0); }
    if (s) { show_status(); host_exit(0); }
    if (t) { test_mode(1); host_exit(0); }
    if (m) { test_motion(); host_exit(0); }
    if (sl) { test_slam(); host_exit(0); }
    if (c) { test_ctrl(); host_exit(0); }
    if (f) { test_multi(); host_exit(0); }
