...
void _start(void)
{
    ...
    if (i >= argv_len) {
        const char *msg = "usage: rmdir <name>\n";
        unsigned int off = alloc(18);
        copy_to_mem(off, msg, 18);
        host_print(off, 18);
        host_exit(1);
    }
    ...
    int rc = host_fs_delete((unsigned int)(name - argv), name_len);

    char buf[64];
    unsigned int pos = 0;
    if (rc >= 0) {
        const char *ok = "Directory removed: ";
        while (*ok) buf[pos++] = *ok++;
    } else {
        const char *err = "Failed to remove directory\n";
