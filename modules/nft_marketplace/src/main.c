 ...
 *
 * AI-ASM OS module. Pure C, no stdlib. Implements ERC721/ERC1155 style tokens,
 * English/Dutch/buy-now auctions, royalty splits, IPFS metadata and market stats.
 */
...
typedef struct {
    ...
    char         seller[20];
    int          auction_type;
    unsigned int start_price;
    unsigned int reserve_price;
    unsigned int end_price;          /* dutch only */
...
typedef struct {
    unsigned int token_id;
    unsigned int original_sale_price;
    unsigned int secondary_sale_price;
    unsigned int royalty_amount;
    char         creator[20];
} royalty_info_t;
...
typedef struct {
    ...
    char         image_uri[MAX_IMAGE_LEN];
    attribute_t  attributes[MAX_ATTRS];
    int          attribute_count;
    int          media_type;
} metadata_t;

typedef struct {
    unsigned int floor_price;
...
/* approval table: token_id -> approved address */
...
static unsigned int my_max(unsigned int a, unsigned int b) { return a > b ? a : b; }
static unsigned int my_min(unsigned int a, unsigned int b) { return a < b ? a : b; }
...
static int nft_mint(int col_idx, const char *creator, const char *uri,
    ...
    t->token_id = token_id;
    addr_copy(t->owner, creator);
    addr_copy(t->creator, creator);
...
static int nft_approve(unsigned int token_id, const char *owner, const char *approved) {
    ...
    if (!addr_eq(t->owner, owner)) return ERR_OWNER;
    for (unsigned int i = 0; i < g_approval_count; i++) {
        if (g_approval_tid[i] == token_id) { addr_copy(g_approval_addr[i], approved); return OK; }
    }
...
static int auction_create_english(unsigned int token_id, const char *seller,
                                  unsigned int start_price, unsigned int reserve,
                                  unsigned int duration_ms) {
    ...
    a->token_id = token_id;
    addr_copy(a->seller, seller);
    a->auction_type = AUCTION_ENGLISH;
    a->start_price = start_price;
    a->reserve_price = reserve;
    a->end_price = 0;
    a->current_bid = 0;
    a->highest_bidder[0] = '\0';
    a->duration_ms = duration_ms;
    a->start_time = 0;
    a->end_time = duration_ms;
    a->status = AUCTION_ACTIVE;
    return (int)(g_auction_count - 1);
}

static int auction_create_dutch(unsigned int token_id, const char *seller,
                                unsigned int start_price, unsigned int end_price,
                                unsigned int duration_ms) {
    if (g_auction_count >= MAX_AUCTIONS) return ERR_CAPACITY;
    if (!nft_find(token_id)) return ERR_NOT_FOUND;
    auction_t *a = &g_auctions[g_auction_count++];
...
static int auction_create_buy_now(unsigned int token_id, const char *seller,
                                  unsigned int price) {
    if (g_auction_count >= MAX_AUCTIONS) return ERR_CAPACITY;
    if (!nft_find(token_id)) return ERR_NOT_FOUND;
    auction_t *a = &g_auctions[g_auction_count++];
    a->token_id = token_id;
    addr_copy(a->seller, seller);
    a->auction_type = AUCTION_BUY_NOW;
    a->start_price = price;
    a->reserve_price = price;
    a->end_price = price;
    a->current_bid = 0;
    a->highest_bidder[0] = '\0';
    a->duration_ms = 0;
...
static int auction_place_bid(int auc_idx, const char *bidder, unsigned int amount) {
    ...
    addr_copy(a->highest_bidder, bidder);
    return OK;
}
...
static unsigned int auction_tick_dutch(int auc_idx, unsigned int elapsed_ms) {
    ...
    unsigned int current = a->start_price - (drop * elapsed_ms / a->duration_ms);
    return current;
}
...
static int auction_buy_now(int auc_idx, const char *buyer) {
    ...
    if (a->auction_type != AUCTION_BUY_NOW) return ERR_STATE;
    addr_copy(a->highest_bidder, buyer);
    a->current_bid = a->start_price;
...
static int auction_settle(int auc_idx) {
    ...
    addr_copy(ev.from, a->seller);
    addr_copy(ev.to, a->highest_bidder);
    market_record_event(&g_market_log, &ev);
    return OK;
}
...
static unsigned int royalty_compute(unsigned int original_price, unsigned int resale_price,
                                    unsigned int royalty_pct) {
...
static int royalty_distribute(const royalty_info_t *info) {
    ...
    print_str(" paid to creator for token "); print_int((int)info->token_id);
    print_str("\n");
    return OK;
}
...
static int metadata_detect_media(const char *uri) {
    ...
    if (my_strncmp(ext, ".jpg", 4) == 0 || my_strncmp(ext, ".png", 4) == 0 ||
        my_strncmp(ext, "jpeg", 4) == 0 || my_strncmp(ext, ".gif", 4) == 0) return MEDIA_IMAGE;
    if (my_strncmp(ext, ".mp4", 4) == 0 || my_strncmp(ext, "webm", 4) == 0) return MEDIA_VIDEO;
    if (my_strncmp(ext, ".mp3", 4) == 0 || my_strncmp(ext, "ogg", 4) == 0) return MEDIA_AUDIO;
    if (my_strncmp(ext, ".glb", 4) == 0 || my_strncmp(ext, "gltf", 4) == 0) return MEDIA_3D;
    return MEDIA_UNKNOWN;
}

static int metadata_validate(const metadata_t *m) {
    if (!m) return ERR_INVALID;
    if (m->name[0] == '\0') return ERR_INVALID;
    if (m->image_uri[0] == '\0') return ERR_INVALID;
    return OK;
}
...
static metadata_t metadata_parse(const char *json, int len) {
    ...
    /* simplified key-value extraction: look for "name":"value" */
    for (int i = 0; i < len - 10; i++) {
        if (json[i] == '"' && my_strncmp(json + i + 1, "name\"", 5) == 0) {
            ...
            int n = e - s; if (n > MAX_NAME_LEN - 1) n = MAX_NAME_LEN - 1;
        ...
        if (json[i] == '"' && my_strncmp(json + i + 1, "image\"", 6) == 0) {
            ...
            int e = s;
            while (e < len && json[e] != '"') e++;
            int n = e - s; if (n > MAX_IMAGE_LEN - 1) n = MAX_IMAGE_LEN - 1;
...
static void metadata_ipfs_link(const char *cid, char *out, int max_len) {
    ...
    int off = plen;
    for (int i = 0; i < clen && off < max_len - 1; i++, off++) out[off] = cid[i];
    out[off] = '\0';
}
...
static int collection_create(const char *name, const char *symbol, unsigned int max_supply) {
...
static collection_stats_t collection_compute_stats(int col_idx) {
    ...
    for (unsigned int i = 0; i < g_market_log.count; i++) {
        ...
        nft_token_t *t = nft_find(e->token_id);
        if (!t) continue;
        int in_col = 0;
        for (unsigned int j = 0; j < c->token_count; j++)
...
static unsigned int collection_compute_rarity(unsigned int token_id, int col_idx) {
    if (col_idx < 0 || col_idx >= (int)g_collection_count) return 0;
    /* rarity based on token_id position: lower id = rarer */
    nft_collection_t *c = &g_collections[col_idx];
    if (c->token_count == 0) return 0;
    for (unsigned int i = 0; i < c->token_count; i++) {
        if (c->token_ids[i] == token_id) {
            /* score = inverse position * 1000 */
...
static unsigned int collection_get_holders(int col_idx, char out[][20], unsigned int max_out) {
    if (col_idx < 0 || col_idx >= (int)g_collection_count) return 0;
    nft_collection_t *c = &g_collections[col_idx];
    unsigned int count = 0;
...
static void market_log_init(market_log_t *log) { log->count = 0; log->max_events = MAX_EVENTS; }
...
static unsigned int market_floor_price(int col_idx) {
    collection_stats_t s = collection_compute_stats(col_idx);
    return s.floor_price;
}
...
static unsigned int market_price_history(unsigned int token_id, unsigned int *out,
...
static int market_trending(int *out_col, unsigned int *out_vol, unsigned int max_out) {
    /* simple: sum volume per collection */
    ...
    for (unsigned int i = 0; i < g_market_log.count; i++) {
        ...
        for (unsigned int c = 0; c < g_collection_count; c++) {
            for (unsigned int j = 0; j < g_collections[c].token_count; j++)
                if (g_collections[c].token_ids[j] == e->token_id) { vol[c] += e->price; break; }
        }
    }
    /* sort by volume descending (selection, top max_out) */
    int count = 0;
    for (unsigned int k = 0; k < g_collection_count && count < (int)max_out; k++) {
        ...
        for (unsigned int c = 0; c < g_collection_count; c++) {
            int used = 0;
            for (int p = 0; p < count; p++) if (out_col[p] == (int)c) { used = 1; break; }
            if (!used && vol[c] > best_v) { best_v = vol[c]; best = (int)c; }
        }
        if (best >= 0) { out_col[count] = best; out_vol[count] = best_v; count++; }
    }
    return count;
}
...
static void run_tests(void) {
    ...
    print_str("  Minted 3 art + 1 music (erc1155 x100)\n");
    print_str("  Alice art balance: "); print_int((int)nft_balance_of(art, alice));
    print_str("  Bob art balance:   "); print_int((int)nft_balance_of(art, bob)); print_str("\n");

    /* Approvals and transfers */
    print_str("\n[3] Approvals and transfers\n");
    nft_approve(1, alice, bob);
    print_str("  Alice approved Bob for token 1: ");
    print_int(nft_is_approved(1, bob)); print_str("\n");
    nft_transfer(1, alice, carol);
    print_str("  Token 1 transferred to Carol. Alice balance: ");
    print_int((int)nft_balance_of(art, alice)); print_str("\n");

    /* Royalties */
    print_str("\n[4] Royalty system\n");
    royalty_set(art, 250); /* 2.5% */
    unsigned int roy = royalty_compute(10000, 50000, 250);
    print_str("  2.5% royalty on 50000 resale: "); print_int((int)roy); print_str("\n");
    royalty_info_t ri = royalty_query(1, 50000);
    print_str("  Query token 1 @ 50000 => royalty="); print_int((int)ri.royalty_amount);
    print_str("\n");
    royalty_distribute(&ri);

    /* Metadata */
    print_str("\n[5] Metadata\n");
    const char *json = "{\"name\":\"Cosmic Art\",\"image\":\"ipfs://QmCosmic.png\",\"attrs\":[]}";
    metadata_t md = metadata_parse(json, my_strlen(json));
    print_str("  Parsed name: "); print_str(md.name); print_str("\n");
    print_str("  Media type: "); print_int(md.media_type); print_str(" (0=IMAGE)\n");
    int ok = metadata_validate(&md);
    print_str("  Validate: "); print_int(ok); print_str("\n");
    char ipfs[64]; metadata_ipfs_link("QmAbc123", ipfs, 64);
    print_str("  IPFS link: "); print_str(ipfs); print_str("\n");
    char ser[256]; int sl = metadata_serialize(&md, ser, 256);
    print_str("  Serialized ("); print_int(sl); print_str("B): "); print_str(ser); print_str("\n");

    /* English auction */
    print_str("\n[6] English auction\n");
    int ea = auction_create_english(2, alice, 1000, 5000, 60000);
    print_str("  English auction created idx="); print_int(ea); print_str("\n");
    auction_place_bid(ea, bob, 2000);
    print_str("  Bob bid 2000\n");
    auction_place_bid(ea, carol, 3000);
    print_str("  Carol bid 3000\n");
    int low = auction_place_bid(ea, bob, 2500);
    print_str("  Bob bid 2500 rejected: "); print_int(low); print_str("\n");
    auction_end_english(ea);
    print_str("  Auction settled. Carol owns token 2? balance=");
    print_int((int)nft_balance_of(art, carol)); print_str("\n");

    /* Dutch auction */
    print_str("\n[7] Dutch auction\n");
    int da = auction_create_dutch(3, bob, 10000, 1000, 10000);
    print_str("  Dutch auction idx="); print_int(da);
    print_str(" start=10000 end=1000 dur=10000ms\n");
    print_str("  Price at 0ms:    "); print_int((int)auction_tick_dutch(da, 0)); print_str("\n");
    print_str("  Price at 5000ms: "); print_int((int)auction_tick_dutch(da, 5000)); print_str("\n");
    print_str("  Price at 10000ms:"); print_int((int)auction_tick_dutch(da, 10000)); print_str("\n");

    /* Buy-now */
    print_str("\n[8] Buy-now\n");
    int bn = auction_create_buy_now(10, carol, 7500);
    print_str("  Buy-now idx="); print_int(bn); print_str(" price=7500\n");
    auction_buy_now(bn, alice);
    print_str("  Alice bought token 10 via buy-now\n");

    /* Cancel */
    print_str("\n[9] Auction cancel\n");
    /* mint another and cancel its auction */
    nft_mint(art, alice, "ipfs://QmX.png", 4, STD_ERC721, 0);
    int cx = auction_create_english(4, alice, 500, 0, 30000);
    int cr = auction_cancel(cx, alice);
    print_str("  Cancel auction for token 4: "); print_int(cr); print_str("\n");

    /* Burn */
    print_str("\n[10] Burn\n");
    nft_burn(4, alice);
    print_str("  Token 4 burned. Bob art balance: ");
    print_int((int)nft_balance_of(art, bob)); print_str("\n");

    /* Collection stats */
    print_str("\n[11] Collection stats\n");
    collection_stats_t cs = collection_compute_stats(art);
    print_str("  Art collection: tokens="); print_int((int)cs.token_count);
    print_str(" holders="); print_int((int)cs.holder_count);
    print_str(" volume="); print_int((int)cs.total_volume);
    print_str(" floor="); print_int((int)cs.floor_price); print_str("\n");

    unsigned int rar = collection_compute_rarity(1, art);
    print_str("  Token 1 rarity: "); print_int((int)rar); print_str("\n");

    char holders[4][20];
    unsigned int hc = collection_get_holders(art, holders, 4);
    print_str("  Unique holders: "); print_int((int)hc); print_str("\n");

    /* Market stats */
    print_str("\n[12] Market stats\n");
    print_str("  Floor price (art): "); print_int((int)market_floor_price(art)); print_str("\n");
    print_str("  Volume (art):      "); print_int((int)market_volume(art)); print_str("\n");

    unsigned int hist[8];
    unsigned int hcnt = market_price_history(2, hist, 8);
    print_str("  Token 2 price history: "); print_int((int)hcnt); print_str(" sales\n");

    int tcol[4]; unsigned int tvol[4];
    int tc = market_trending(tcol, tvol, 4);
    print_str("  Trending collections: "); print_int(tc); print_str("\n");
    for (int i = 0; i < tc; i++) {
        print_str("    #"); print_int(i + 1); print_str(" col="); print_int(tcol[i]);
        print_str(" vol="); print_int((int)tvol[i]); print_str("\n");
    }
