/* bearing_admin: Bearing technology administration (v1.0)
 * Rolling bearing, sliding bearing, special bearing, accessories, marketing
 */
#include <stddef.h>
...
int brg_init(void){if(init)return -1;st.n_rl=0;st.n_sl=0;st.n_sp=0;st.n_ac=0;st.n_mk=0;st.t_f1=0;st.t_f2=0;st.t_f3=0;st.t_f4=0;st.t_f5=0;for(int i=0;i<N;i++)rls[i].active=0;for(int i=0;i<N-2;i++)sls[i].active=0;for(int i=0;i<N-4;i++)sps[i].active=0;for(int i=0;i<N-6;i++)acs[i].active=0;for(int i=0;i<N-6;i++)mks[i].active=0;init=1;ps("[BRG] Bearing initialized\n");return 0;}
...
int main(void){
ps("=== Bearing Admin Demo ===\n\n");brg_init();
ps("Rolling bearings...\n");for(int i=0;i<N;i++){int t=(i%5)+1,c=(i%4)+1;brg_rolling(t,c,377+(i*17),362+(i*14),342+(i*10),324+(i*6),2020+(i%5));}
ps("\nSliding bearings...\n");for(int i=0;i<N-2;i++){int t=(i%4)+1,c=(i%5)+1;brg_sliding(t,c,366+(i*15),352+(i*12),334+(i*8),321+(i*5),2021+(i%4));}
ps("\nSpecial bearings...\n");for(int i=0;i<N-4;i++){int t=(i%4)+1,c=(i%5)+1;brg_special(t,c,358+(i*13),344+(i*10),328+(i*7),317+(i*4),2022+(i%3));}
ps("\nBearing accessories...\n");for(int i=0;i<N-6;i++){int t=(i%4)+1,c=(i%5)+1;brg_accessory(t,c,350+(i*11),338+(i*9),324+(i*6),314+(i*3),2023+(i%2));}
ps("\nBearing marketing...\n");for(int i=0;i<N-6;i++){int t=(i%4)+1,c=(i%5)+1;brg_market(t,c,344+(i*9),333+(i*7),320+(i*5),312+(i*3),2024);}
