 ...
 * AI-ASM OS (AArch64 WebAssembly-native OS) - Pure C, NO stdlib */
#include <stddef.h>
...
static void memzero(void *p, unsigned int n) { unsigned char *b = (unsigned char *)p; for (unsigned int i = 0; i < n; i++) b[i] = 0; }
static void mcpy(void *d, const void *s, unsigned int n) { unsigned char *a = (unsigned char *)d; const unsigned char *b = (const unsigned char *)s; for (unsigned int i = 0; i < n; i++) a[i] = b[i]; }
...
static void hash_block_header(const void *hdr, unsigned int sz, unsigned char *out) { sha256((const unsigned char *)hdr, sz, out); }
static void merkle_root(const unsigned char tx_h[][32], unsigned int cnt, unsigned char *out) {
    ...
    while (lc > 1) {
        unsigned int nx = 0;
        for (i = 0; i + 1 < lc; i += 2) { mcpy(buf, level[i], 32); mcpy(buf+32, level[i+1], 32); sha256(buf, 64, level[nx++]); }
...
/* ===== Block Structure ===== */
typedef struct { unsigned int version; unsigned char prev_hash[32]; unsigned char merkle_root[32]; unsigned int timestamp; unsigned int difficulty; unsigned int nonce; } block_header_t;
typedef struct { unsigned char from[20]; unsigned char to[20]; unsigned long long amount; unsigned long long gas_limit; unsigned long long gas_price; unsigned int nonce; unsigned char signature[64]; } transaction_t;
typedef struct { block_header_t header; transaction_t *txs; unsigned int tx_count; unsigned char block_hash[32]; unsigned int height; } block_t;
typedef struct { block_t *blocks; block_t genesis; unsigned int chain_length; unsigned int difficulty; } blockchain_t;
...
static void blockchain_init(blockchain_t *ch) {
    ...
    print_str("[blockchain] Genesis block created: "); print_hex(ch->genesis.block_hash, 8); print_str("\n");
}
static block_t blockchain_create_block(blockchain_t *ch, transaction_t *txs, unsigned int cnt) {
    ...
    blk.header.version = 1; blk.header.timestamp = ch->genesis.header.timestamp + ch->chain_length * 600;
    blk.header.difficulty = ch->difficulty; mcpy(blk.header.prev_hash, ch->genesis.block_hash, 32);
    blk.txs = txs; blk.tx_count = cnt; blk.height = ch->chain_length;
    if (cnt > 0 && txs) {
        unsigned char th[8][32]; unsigned int c = cnt > 8 ? 8 : cnt;
        for (unsigned int i = 0; i < c; i++) sha256((const unsigned char *)&txs[i], sizeof(transaction_t), th[i]);
...
static int blockchain_add_block(blockchain_t *ch, block_t *blk) {
    if (mcmp(blk->header.prev_hash, ch->genesis.block_hash, 32) != 0) { print_str("[blockchain] ERROR: prev_hash mismatch\n"); return 0; }
    hash_block_header(&blk->header, sizeof(block_header_t), blk->block_hash);
    mcpy(ch->genesis.block_hash, blk->block_hash, 32); ch->genesis = *blk; ch->chain_length++; return 1;
}
...
static int consensus_pow(block_t *blk, unsigned int diff) {
    ...
    while (attempts < 100000) {
        ...
        for (unsigned int i = 0; i < 32 && zeros < target; i++) { if (blk->block_hash[i] == 0) zeros++; else break; }
...
static int consensus_pos_validators(const validator_t *vals, const unsigned long long *stakes, unsigned int cnt, unsigned char *sel) {
    unsigned long long total = 0; for (unsigned int i = 0; i < cnt; i++) total += stakes[i];
    if (total == 0) return -1;
    unsigned long long pick = rng_next() % total, acc = 0;
    for (unsigned int i = 0; i < cnt; i++) { acc += stakes[i]; if (pick < acc) { mcpy(sel, vals[i].address, 20); return (int)i; } }
    return -1;
}
typedef struct { unsigned char address[20]; unsigned long long votes; } delegate_t;
static unsigned int consensus_dpos_delegates(const delegate_t *dels, const unsigned long long *votes, unsigned int cnt, unsigned char *win, unsigned int top) {
    unsigned long long sc[32]; unsigned int ord[32]; unsigned int lim = cnt > 32 ? 32 : cnt;
    for (unsigned int i = 0; i < lim; i++) { sc[i] = votes[i]; ord[i] = i; }
    for (unsigned int i = 0; i + 1 < lim; i++) for (unsigned int j = i + 1; j < lim; j++) {
        if (sc[j] > sc[i]) { unsigned long long t = sc[i]; sc[i] = sc[j]; sc[j] = t; unsigned int o = ord[i]; ord[i] = ord[j]; ord[j] = o; }
    }
...
static int consensus_pbft_prepare(const void *prop, unsigned int psz, const pbft_state_t *vals, unsigned int cnt, unsigned int thr) {
    unsigned char ph[32]; sha256((const unsigned char *)prop, psz, ph);
    unsigned int ap = 0; for (unsigned int i = 0; i < cnt; i++) if (vals[i].approved) ap++;
    return ap >= thr ? 1 : 0;
}
static int consensus_pbft_commit(pbft_state_t *ph, unsigned int cnt) {
    unsigned int c = 0; for (unsigned int i = 0; i < cnt; i++) { ph[i].phase = 2; if (ph[i].approved) c++; }
    return c == cnt ? 1 : 0;
}
static unsigned int consensus_get_difficulty(const blockchain_t *ch) {
    if (ch->chain_length > 20) return ch->difficulty + 1; return ch->difficulty;
}
...
static transaction_t tx_create(const unsigned char *from, const unsigned char *to, unsigned long long amt, unsigned int nonce) {
    ...
    mcpy(tx.from, from, 20); mcpy(tx.to, to, 20); tx.amount = amt; tx.nonce = nonce; tx.gas_limit = 21000; tx.gas_price = 20;
    return tx;
}
static void tx_sign(transaction_t *tx, const unsigned char *pk) {
    unsigned char msg[32]; sha256((const unsigned char *)tx, sizeof(transaction_t), msg); ecdsa_sign(pk, msg, tx->signature);
}
static int tx_verify(const transaction_t *tx) {
    unsigned char msg[32], pub[33]; mcpy(pub, tx->from, 20); for (int i = 20; i < 33; i++) pub[i] = 0;
    sha256((const unsigned char *)tx, sizeof(transaction_t), msg); return ecdsa_verify(pub, msg, tx->signature);
}
static unsigned long long tx_compute_gas(const transaction_t *tx) { return tx->gas_limit * tx->gas_price; }
...
static int tx_pool_add(tx_pool_t *p, const transaction_t *tx) { if (p->count >= 32) return -1; p->pending[p->count++] = *tx; return (int)p->count; }
static unsigned int tx_pool_get_batch(const tx_pool_t *p, transaction_t *out, unsigned int mx) {
    unsigned int n = mx > p->count ? p->count : mx; for (unsigned int i = 0; i < n; i++) out[i] = p->pending[i]; return n;
}

/* ===== Smart Contract (EVM-like) ===== */
typedef struct { unsigned char address[20]; unsigned char *bytecode; unsigned int bytecode_len; unsigned long long balance; unsigned char storage[16][32]; unsigned int storage_count; } contract_t;
typedef struct { long long stack[256]; int stack_ptr; unsigned char memory[256]; unsigned int pc; long long gas_remaining; unsigned char return_data[32]; int halted; } evm_state_t;

static void evm_init(evm_state_t *st, const contract_t *c, long long gas) {
    memzero(st, sizeof(evm_state_t)); st->gas_remaining = gas;
    if (c->bytecode && c->bytecode_len > 0) { unsigned int n = c->bytecode_len > 256 ? 256 : c->bytecode_len; mcpy(st->memory, c->bytecode, n); }
}
static int evm_execute(evm_state_t *st) {
    unsigned int steps = 0;
    while (!st->halted && steps < 1024 && st->gas_remaining > 0) {
        unsigned char op = st->memory[st->pc]; st->gas_remaining -= 3; st->pc++; steps++;
        if (op == 0x00) { st->halted = 1; }                                  /* STOP */
            ...
            long long v = ((long long)st->memory[st->pc] << 8) | (long long)st->memory[st->pc+1]; st->pc += 2;
            if (st->stack_ptr < 256) st->stack[st->stack_ptr++] = v;
        } else if (op == 0x50) { if (st->stack_ptr > 0) st->stack_ptr--; }   /* POP */
        else if (op == 0x01 && st->stack_ptr >= 2) {                          /* ADD */
            long long b = st->stack[--st->stack_ptr], a = st->stack[--st->stack_ptr]; st->stack[st->stack_ptr++] = a + b;
        ...
        } else if (op == 0xF1) { if (st->stack_ptr >= 1) st->stack_ptr--; st->gas_remaining -= 100; } /* CALL */
        else if (op == 0xF3 && st->stack_ptr >= 1) {                          /* RETURN */
            long long v = st->stack[--st->stack_ptr];
            for (unsigned int i = 0; i < 8; i++) st->return_data[i] = (unsigned char)((v >> (i*8)) & 0xFF);
            st->halted = 1;
        }
    }
    return steps;
}
static void evm_deploy(const unsigned char *dep, const unsigned char *bc, unsigned int bl, long long gas, unsigned char *addr) {
    unsigned char buf[84]; mcpy(buf, dep, 20); unsigned int n = bl > 64 ? 64 : bl; mcpy(buf+20, bc, n);
    sha256(buf, 20 + n, buf); mcpy(addr, buf, 20);
}
static long long evm_call(const unsigned char *caller, const unsigned char *caddr, const unsigned char *data, unsigned int dl, long long gas) {
    unsigned char buf[128]; mcpy(buf, caller, 20); mcpy(buf+20, caddr, 20);
    unsigned int n = dl > 88 ? 88 : dl; if (n > 0) mcpy(buf+40, data, n);
    contract_t c; memzero(&c, sizeof(c)); c.bytecode = buf; c.bytecode_len = 40 + n;
    evm_state_t st; evm_init(&st, &c, gas); evm_execute(&st); return st.gas_remaining;
}
...
static state_trie_t *trie_init(void) {
    state_trie_t *t = (state_trie_t *)alloc_mem(sizeof(state_trie_t)); memzero(t, sizeof(state_trie_t));
    t->root = &t->nodes[t->node_count++]; t->root->type = TRIE_BRANCH; return t;
}
static int trie_insert(state_trie_t *t, const unsigned char *k, unsigned int kl, const unsigned char *v, unsigned int vl) {
    ...
    trie_node_t *nd = &t->nodes[t->node_count++]; memzero(nd, sizeof(trie_node_t)); nd->type = TRIE_LEAF;
...
/* ===== Wallet ===== */
typedef struct { unsigned char address[20]; unsigned char public_key[33]; unsigned char private_key[32]; unsigned long long balance; unsigned int nonce; } wallet_t;
...
static wallet_t wallet_create(void) {
    ...
    mcpy(w.address, pkh, 20); w.balance = 1000000; return w;
}
static unsigned long long wallet_get_balance(const wallet_t *w) { return w->balance; }
static transaction_t wallet_send(wallet_t *w, const unsigned char *to, unsigned long long amt, unsigned long long gas) {
    transaction_t tx = tx_create(w->address, to, amt, w->nonce);
    tx.gas_limit = gas > 0 ? gas : 21000; tx.gas_price = 20; tx_sign(&tx, w->private_key); w->nonce++;
    unsigned long long cost = amt + tx_compute_gas(&tx); if (w->balance >= cost) w->balance -= cost;
    return tx;
}
static void wallet_sign_message(const wallet_t *w, const unsigned char *msg, unsigned int ml, unsigned char *sig) {
    unsigned char mh[32]; sha256(msg, ml, mh); ecdsa_sign(w->private_key, mh, sig);
}
...
static void test_blockchain(void) {
    ...
    tx_sign(&tx, pk); print_str("  tx gas="); print_int((int)tx_compute_gas(&tx)); print_str("\n\n");

    print_str("[4] PoW Mining:\n  ");
    block_t nb = blockchain_create_block(&chain, &tx, 1);
    int mined = consensus_pow(&nb, chain.difficulty);
    print_str("  mined="); print_int(mined); print_str(", nonce="); print_int((int)nb.header.nonce); print_str("\n\n");
    ...
    print_str("  steps="); print_int(steps); print_str(", gas_remaining="); print_int((int)est.gas_remaining); print_str("\n\n");
    ...
    transaction_t wtx = wallet_send(&w, ta, 100, 21000);
    ...
    unsigned char txh[2][32]; sha256((const unsigned char *)"tx1", 3, txh[0]); sha256((const unsigned char *)"tx2", 3, txh[1]);
...
void _start(void) {
    ...
    else { print_str("Blockchain Core v57.0 - Decentralized Ledger Engine\n"); print_str("Use -h for help, -t to run tests\n"); }
    host_exit(0);
}
