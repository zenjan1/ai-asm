/* forgingtech_admin: Forging technology administration (v1.0)
...
int frg_init(void){if(init)return -1;st.n_fr=0;st.n_df=0;st.n_sf=0;st.n_ac=0;st.n_mk=0;st.t_f1=0;st.t_f2=0;st.t_f3=0;st.t_f4=0;st.t_f5=0;for(int i=0;i<N;i++)frs[i].active=0;for(int i=0;i<N-2;i++)dfs[i].active=0;for(int i=0;i<N-4;i++)sfs[i].active=0;for(int i=0;i<N-6;i++)acs[i].active=0;for(int i=0;i<N-6;i++)mks[i].active=0;init=1;ps("[FRG] Forgingtech initialized\n");return 0;}
...
ps("Free forging...\n");for(int i=0;i<N;i++){int t=(i%5)+1,c=(i%4)+1;frg_free(t,c,269+(i*17),254+(i*14),234+(i*10),216+(i*6),2020+(i%5));}
ps("\nDie forging...\n");for(int i=0;i<N-2;i++){int t=(i%4)+1,c=(i%5)+1;frg_die(t,c,258+(i*15),244+(i*12),226+(i*8),213+(i*5),2021+(i%4));}
ps("\nSpecial forging...\n");for(int i=0;i<N-4;i++){int t=(i%4)+1,c=(i%5)+1;frg_special(t,c,250+(i*13),236+(i*10),220+(i*7),209+(i*4),2022+(i%3));}
ps("\nForging accessories...\n");for(int i=0;i<N-6;i++){int t=(i%4)+1,c=(i%5)+1;frg_accessory(t,c,242+(i*11),230+(i*9),216+(i*6),206+(i*3),2023+(i%2));}
ps("\nForging marketing...\n");for(int i=0;i<N-6;i++){int t=(i%4)+1,c=(i%5)+1;frg_market(t,c,236+(i*9),225+(i*7),212+(i*5),204+(i*3),2024);}
