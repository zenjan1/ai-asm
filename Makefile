...
NAMEIF_WASM      = $(MODULES_DIR)/nameif/nameif.wasm $(MODULES_DIR)/plipconfig/plipconfig.wasm $(MODULES_DIR)/pppd/pppd.wasm $(MODULES_DIR)/slattach/slattach.wasm
PLIPCONFIG_WASM  = $(MODULES_DIR)/plipconfig/plipconfig.wasm $(MODULES_DIR)/pppd/pppd.wasm $(MODULES_DIR)/slattach/slattach.wasm
PPPD_WASM        = $(MODULES_DIR)/pppd/pppd.wasm $(MODULES_DIR)/slattach/slattach.wasm
SLATTACH_WASM    = $(MODULES_DIR)/slattach/slattach.wasm
...
# ---------------------------------------------------------------------------
$(PING6_WASM) $(ETHTOOL_WASM) $(ARPING_WASM) $(BRCTL_WASM) $(IFSTAT_WASM) $(MII_TOOL_WASM) $(NAMEIF_WASM) $(PLIPCONFIG_WASM) $(PPPD_WASM) $(SLATTACH_WASM) $(RAMDISK_TAR): ramdisk/motd.txt ramdisk/hello.txt ramdisk/readme.txt
	@echo "  TAR   $@"
...
# Full kernel link
# ---------------------------------------------------------------------------
$(TARGET_ELF): $(ALL_OBJS) $(WASM3_LIB) $(INIT_WASM) $(SHELL_WASM) $(TEST_WASM) $(PING6_WASM) $(PPPD_WASM) $(SLATTACH_WASM) $(RAMDISK_TAR)
	@echo "  LD    $@"
