...
GZIP_WASM       = $(MODULES_DIR)/gzip/gzip.wasm $(MODULES_DIR)/gunzip/gunzip.wasm $(MODULES_DIR)/zcat/zcat.wasm $(MODULES_DIR)/bzip2/bzip2.wasm $(MODULES_DIR)/bunzip2/bunzip2.wasm $(MODULES_DIR)/lzma/lzma.wasm $(MODULES_DIR)/unlzma/unlzma.wasm $(MODULES_DIR)/xz/xz.wasm $(MODULES_DIR)/column/column.wasm
...
XZ_WASM         = $(MODULES_DIR)/xz/xz.wasm $(MODULES_DIR)/column/column.wasm
...
COLUMN_WASM     = $(MODULES_DIR)/column/column.wasm
...
$(PR_WASM): $(MODULES_DIR)/pr/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/pr.wasm $(KERNEL_DIR)/column.wasm
