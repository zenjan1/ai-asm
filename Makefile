...
GZIP_WASM       = $(MODULES_DIR)/gzip/gzip.wasm $(MODULES_DIR)/gunzip/gunzip.wasm $(MODULES_DIR)/zcat/zcat.wasm $(MODULES_DIR)/bzip2/bzip2.wasm $(MODULES_DIR)/bunzip2/bunzip2.wasm $(MODULES_DIR)/lzma/lzma.wasm $(MODULES_DIR)/unlzma/unlzma.wasm $(MODULES_DIR)/xz/xz.wasm $(MODULES_DIR)/column/column.wasm $(MODULES_DIR)/expand/expand.wasm $(MODULES_DIR)/unexpand/unexpand.wasm $(MODULES_DIR)/numfmt/numfmt.wasm
...
XZ_WASM         = $(MODULES_DIR)/xz/xz.wasm $(MODULES_DIR)/column/column.wasm $(MODULES_DIR)/expand/expand.wasm $(MODULES_DIR)/unexpand/unexpand.wasm $(MODULES_DIR)/numfmt/numfmt.wasm
...
COLUMN_WASM     = $(MODULES_DIR)/column/column.wasm $(MODULES_DIR)/expand/expand.wasm $(MODULES_DIR)/unexpand/unexpand.wasm $(MODULES_DIR)/numfmt/numfmt.wasm
EXPAND_WASM      = $(MODULES_DIR)/expand/expand.wasm $(MODULES_DIR)/unexpand/unexpand.wasm $(MODULES_DIR)/numfmt/numfmt.wasm
UNEXPAND_WASM    = $(MODULES_DIR)/unexpand/unexpand.wasm $(MODULES_DIR)/numfmt/numfmt.wasm
NUMFMT_WASM      = $(MODULES_DIR)/numfmt/numfmt.wasm
...
$(PR_WASM): $(MODULES_DIR)/pr/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/pr.wasm $(KERNEL_DIR)/column.wasm $(KERNEL_DIR)/expand.wasm $(KERNEL_DIR)/unexpand.wasm $(KERNEL_DIR)/numfmt.wasm
...
$(COLUMN_WASM): $(MODULES_DIR)/column/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/column.wasm $(KERNEL_DIR)/expand.wasm $(KERNEL_DIR)/unexpand.wasm $(KERNEL_DIR)/numfmt.wasm
...
$(EXPAND_WASM): $(MODULES_DIR)/expand/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/expand.wasm $(KERNEL_DIR)/unexpand.wasm $(KERNEL_DIR)/numfmt.wasm
...
$(UNEXPAND_WASM): $(MODULES_DIR)/unexpand/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/unexpand.wasm $(KERNEL_DIR)/numfmt.wasm
