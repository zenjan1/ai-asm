...
STRACE_WASM       = $(MODULES_DIR)/strace/strace.wasm $(MODULES_DIR)/ltrace/ltrace.wasm $(MODULES_DIR)/tracepath/tracepath.wasm $(MODULES_DIR)/ss/ss.wasm $(MODULES_DIR)/ip/ip.wasm $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
LTRACE_WASM       = $(MODULES_DIR)/ltrace/ltrace.wasm $(MODULES_DIR)/tracepath/tracepath.wasm $(MODULES_DIR)/ss/ss.wasm $(MODULES_DIR)/ip/ip.wasm $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
TRACEPATH_WASM    = $(MODULES_DIR)/tracepath/tracepath.wasm $(MODULES_DIR)/ss/ss.wasm $(MODULES_DIR)/ip/ip.wasm $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
SS_WASM          = $(MODULES_DIR)/ss/ss.wasm $(MODULES_DIR)/ip/ip.wasm $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
IP_WASM          = $(MODULES_DIR)/ip/ip.wasm $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
KILLALL_WASM     = $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
NMAP_WASM        = $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
TAR_WASM         = $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm
DIG_WASM         = $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm $(MODULES_DIR)/host/host.wasm
HOST_CMD_WASM    = $(MODULES_DIR)/host/host.wasm
...
# ---------------------------------------------------------------------------
$(HOST_CMD_WASM) $(RAMDISK_TAR): ramdisk/motd.txt ramdisk/hello.txt ramdisk/readme.txt
	@echo "  TAR   $@"
...
$(MODULES_DIR)/ip/ip.wasm $(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm: $(MODULES_DIR)/ip/src/main.c
	@echo "  WASM  $<"
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/ip.wasm

$(MODULES_DIR)/killall/killall.wasm $(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm: $(MODULES_DIR)/killall/src/main.c
	@echo "  WASM  $<"
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/killall.wasm

$(MODULES_DIR)/nmap/nmap.wasm $(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm: $(MODULES_DIR)/nmap/src/main.c
	@echo "  WASM  $<"
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/nmap.wasm

$(MODULES_DIR)/tar/tar.wasm $(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm: $(MODULES_DIR)/tar/src/main.c
	@echo "  WASM  $<"
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/tar.wasm

$(MODULES_DIR)/dig/dig.wasm $(MODULES_DIR)/host/host.wasm: $(MODULES_DIR)/dig/src/main.c
	@echo "  WASM  $<"
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/dig.wasm

$(MODULES_DIR)/host/host.wasm: $(MODULES_DIR)/host/src/main.c
	@echo "  WASM  $<"
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
...
# Full kernel link
# ---------------------------------------------------------------------------
$(TARGET_ELF): $(ALL_OBJS) $(WASM3_LIB) $(INIT_WASM) $(SHELL_WASM) $(TEST_WASM) $(HOST_CMD_WASM) $(RAMDISK_TAR)
	@echo "  LD    $@"
