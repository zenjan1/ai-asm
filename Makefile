...
PING6_WASM       = $(MODULES_DIR)/ping6/ping6.wasm $(MODULES_DIR)/ethtool/ethtool.wasm $(MODULES_DIR)/arping/arping.wasm $(MODULES_DIR)/brctl/brctl.wasm $(MODULES_DIR)/ifstat/ifstat.wasm
ETHTOOL_WASM     = $(MODULES_DIR)/ethtool/ethtool.wasm $(MODULES_DIR)/arping/arping.wasm $(MODULES_DIR)/brctl/brctl.wasm $(MODULES_DIR)/ifstat/ifstat.wasm
ARPING_WASM      = $(MODULES_DIR)/arping/arping.wasm $(MODULES_DIR)/brctl/brctl.wasm $(MODULES_DIR)/ifstat/ifstat.wasm
BRCTL_WASM       = $(MODULES_DIR)/brctl/brctl.wasm $(MODULES_DIR)/ifstat/ifstat.wasm
IFSTAT_WASM      = $(MODULES_DIR)/ifstat/ifstat.wasm
...
# ---------------------------------------------------------------------------
$(PING6_WASM) $(ETHTOOL_WASM) $(ARPING_WASM) $(BRCTL_WASM) $(IFSTAT_WASM) $(RAMDISK_TAR): ramdisk/motd.txt ramdisk/hello.txt ramdisk/readme.txt
	@echo "  TAR   $@"
...
# Full kernel link
# ---------------------------------------------------------------------------
$(TARGET_ELF): $(ALL_OBJS) $(WASM3_LIB) $(INIT_WASM) $(SHELL_WASM) $(TEST_WASM) $(PING6_WASM) $(IFSTAT_WASM) $(RAMDISK_TAR)
	@echo "  LD    $@"
