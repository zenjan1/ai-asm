...
SETKEYCODES_WASM = $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm $(MODULES_DIR)/dumpkeys/dumpkeys.wasm $(MODULES_DIR)/showkey/showkey.wasm $(MODULES_DIR)/chvt/chvt.wasm $(MODULES_DIR)/openvt/openvt.wasm $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
LOADKEYS_WASM    = $(MODULES_DIR)/loadkeys/loadkeys.wasm $(MODULES_DIR)/dumpkeys/dumpkeys.wasm $(MODULES_DIR)/showkey/showkey.wasm $(MODULES_DIR)/chvt/chvt.wasm $(MODULES_DIR)/openvt/openvt.wasm $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
DUMPKEYS_WASM    = $(MODULES_DIR)/dumpkeys/dumpkeys.wasm $(MODULES_DIR)/showkey/showkey.wasm $(MODULES_DIR)/chvt/chvt.wasm $(MODULES_DIR)/openvt/openvt.wasm $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
SHOWKEY_WASM     = $(MODULES_DIR)/showkey/showkey.wasm $(MODULES_DIR)/chvt/chvt.wasm $(MODULES_DIR)/openvt/openvt.wasm $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
CHVT_WASM        = $(MODULES_DIR)/chvt/chvt.wasm $(MODULES_DIR)/openvt/openvt.wasm $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
OPENVT_WASM      = $(MODULES_DIR)/openvt/openvt.wasm $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
DEALLOCVT_WASM   = $(MODULES_DIR)/deallocvt/deallocvt.wasm $(MODULES_DIR)/fgconsole/fgconsole.wasm
FGCONSOLE_WASM   = $(MODULES_DIR)/fgconsole/fgconsole.wasm
FGCONSOLE_WASM   = $(MODULES_DIR)/fgconsole/fgconsole.wasm
...
$(LOADKEYS_WASM): $(MODULES_DIR)/loadkeys/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/loadkeys.wasm $(KERNEL_DIR)/dumpkeys.wasm $(KERNEL_DIR)/showkey.wasm $(KERNEL_DIR)/chvt.wasm $(KERNEL_DIR)/openvt.wasm $(KERNEL_DIR)/deallocvt.wasm $(KERNEL_DIR)/fgconsole.wasm
...
$(DUMPKEYS_WASM): $(MODULES_DIR)/dumpkeys/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/dumpkeys.wasm $(KERNEL_DIR)/showkey.wasm $(KERNEL_DIR)/chvt.wasm $(KERNEL_DIR)/openvt.wasm $(KERNEL_DIR)/deallocvt.wasm $(KERNEL_DIR)/fgconsole.wasm
...
$(OPENVT_WASM): $(MODULES_DIR)/openvt/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/openvt.wasm $(KERNEL_DIR)/deallocvt.wasm $(KERNEL_DIR)/fgconsole.wasm
...
$(DEALLOCVT_WASM): $(MODULES_DIR)/deallocvt/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/deallocvt.wasm $(KERNEL_DIR)/fgconsole.wasm
...
# Full kernel link
# ---------------------------------------------------------------------------
$(TARGET_ELF): $(ALL_OBJS) $(WASM3_LIB) $(INIT_WASM) $(SHELL_WASM) $(TEST_WASM) $(PING6_WASM) $(PPPD_WASM) $(SLATTACH_WASM) $(SLIPATTACH_WASM) $(SETKEYCODES_WASM) $(LOADKEYS_WASM) $(DUMPKEYS_WASM) $(RAMDISK_TAR)
	@echo "  LD    $@"
