...
NAMEIF_WASM      = $(MODULES_DIR)/nameif/nameif.wasm $(MODULES_DIR)/plipconfig/plipconfig.wasm $(MODULES_DIR)/pppd/pppd.wasm $(MODULES_DIR)/slattach/slattach.wasm $(MODULES_DIR)/slipattach/slipattach.wasm $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm
PLIPCONFIG_WASM  = $(MODULES_DIR)/plipconfig/plipconfig.wasm $(MODULES_DIR)/pppd/pppd.wasm $(MODULES_DIR)/slattach/slattach.wasm $(MODULES_DIR)/slipattach/slipattach.wasm $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm
PPPD_WASM        = $(MODULES_DIR)/pppd/pppd.wasm $(MODULES_DIR)/slattach/slattach.wasm $(MODULES_DIR)/slipattach/slipattach.wasm $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm
SLATTACH_WASM    = $(MODULES_DIR)/slattach/slattach.wasm $(MODULES_DIR)/slipattach/slipattach.wasm $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm
SLIPATTACH_WASM  = $(MODULES_DIR)/slipattach/slipattach.wasm $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm
SETKEYCODES_WASM = $(MODULES_DIR)/setkeycodes/setkeycodes.wasm $(MODULES_DIR)/loadkeys/loadkeys.wasm
LOADKEYS_WASM    = $(MODULES_DIR)/loadkeys/loadkeys.wasm
...
# ---------------------------------------------------------------------------
$(PING6_WASM) $(ETHTOOL_WASM) $(ARPING_WASM) $(BRCTL_WASM) $(IFSTAT_WASM) $(MII_TOOL_WASM) $(NAMEIF_WASM) $(PLIPCONFIG_WASM) $(PPPD_WASM) $(SLATTACH_WASM) $(SLIPATTACH_WASM) $(SETKEYCODES_WASM) $(LOADKEYS_WASM) $(RAMDISK_TAR): ramdisk/motd.txt ramdisk/hello.txt ramdisk/readme.txt
	@echo "  TAR   $@"
...
$(SETKEYCODES_WASM): $(MODULES_DIR)/setkeycodes/src/main.c
	...
	$(CLANG) --target=wasm32-unknown-unknown -Oz -nostdlib -fno-builtin \
	    -Wl,--no-entry -Wl,--export=_start -o $@ $<
	cp $@ $(KERNEL_DIR)/setkeycodes.wasm $(KERNEL_DIR)/loadkeys.wasm
...
# Full kernel link
# ---------------------------------------------------------------------------
$(TARGET_ELF): $(ALL_OBJS) $(WASM3_LIB) $(INIT_WASM) $(SHELL_WASM) $(TEST_WASM) $(PING6_WASM) $(PPPD_WASM) $(SLATTACH_WASM) $(SLIPATTACH_WASM) $(SETKEYCODES_WASM) $(LOADKEYS_WASM) $(RAMDISK_TAR)
	@echo "  LD    $@"
